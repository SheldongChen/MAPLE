...
class VATLoss(nn.Module):
    ...
    def __init__(self, xi=1e-2, eps=0.1, ip=1):
        """VAT loss
...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, data_loader_unlabel, device, epoch, print_freq, xi=1e-2, eps=1.0, ip=1, alpha=1.0, only_ul=False):
    model.train()
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, data_loader_unlabel, device, epoch, args.print_freq, args.vat_xi, args.vat_eps, args.vat_ip, args.vat_alpha, args.vat_only_ul)
