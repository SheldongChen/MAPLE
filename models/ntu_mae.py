...
def ShuffleIndex(index: list, sample_ratio: float):
    if len(index) < 4:
        ...
        sample_length = int(sample_ratio * len(index) + 0.001)
        ...
        assert len(sample_list) == int(len(index) * sample_ratio + 0.001), "sample length must be same as the ratio!!!"
    return sorted(sample_list), sorted(mask_list)

def CalculateIndex(token_length, mask_ratio):
    ...
    return sample_index, mask_index
...
def MaskEmbeeding_With_Index(token_emb, sample_index, mask_index):
    """get the mask embeeding with sample_index and mask_index
    """
    # token_emb: [Bs, Length, Channels]
...
class P4Transformer(nn.Module):
    ...
    def forward(self, input_clips, forward_type='cls', sample_index=None, mask_index=None): # input: [B, L, N, 3]
        assert forward_type in ['cls', 'mask', 'both', 'cls_double']
        ...
        # if forward_type == 'cls_double':
        #     assert sample_index == None
        #     assert mask_index == None
        ...
        #     token_length = torch.tensor(embedding_xyz.shape[1]).to(output_cls.device).reshape(1, -1)
        #     return output_cls, token_length
        ...
        if forward_type == 'cls':
            ...
            mask_embedding_xyz, sample_index, mask_index = MaskEmbeeding_With_Index(embedding_xyz, sample_index, mask_index)
            ...
            return restruct_embedding_xyz, embedding_xyz_clone, output_cls#, mask_index, output_cls
        else:
