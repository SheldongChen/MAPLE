...
class P4Transformer(nn.Module):
    ...
    def forward(self, input):                                                                                                               # [B, L, N, 3]
        ...
        embedding_1 = self.transformer_1(embedding)
        embedding_1 = torch.reshape(input=embedding_1, shape=(B, L, n, C))
        embedding_1 = torch.max(input=embedding_1, dim=-2, keepdim=False, out=None)[0]   # [B, L, C]

        output = self.transformer_2(embedding_1)
